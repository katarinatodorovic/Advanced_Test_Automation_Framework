...
public class VerifyNavigationForEndUser extends BaseTestClass {
    ...
    /**
     * Test that login End user and verify if:
     * @description End user shouldn't have Admin tab on navigation bar
     * 1. every tab title is displayed
     * (Samsara, Home, Users, Heroes, Gallery, API, Practice, Broken Link, Profile, Log Out)
     ...
     */
    @Test
    public void testVerifyNavigationForEndUser()  {

        log.debug(String.format("[START TEST] %s", sTestName));
        LoginPage loginPage = new LoginPage(driver).open();

        WelcomePage welcomePage = loginPage.login(sEndUserUsername,sEndUserPassword);
        ...
        //Admin tab shouldn't be displayed
        softAssert.assertFalse (welcomePage.isAdminTabDisplayed(),"Admin tab should NOT be displayed on navigation bar for End user!");
